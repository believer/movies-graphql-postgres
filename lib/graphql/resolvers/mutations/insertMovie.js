...
module.exports = async (_, { input }) => {
  ...
  try {
    ...
    const movieConflict = 'ON CONFLICT ON CONSTRAINT movies_imdb_id_key DO UPDATE SET imdb_id=$1 RETURNING *;'
    const insertMovieSql = pgp.helpers.insert(remappedMovie, columns)

    // Insert movie
    const insertedMovie = await db.one(`${insertMovieSql} ${movieConflict}`, [remappedMovie.imdb_id])

    const findRow = async (tableName, name) => {
      let row = await db.one(`SELECT id FROM ${tableName} WHERE name = $1`, name)

      if (!row.id) {
        const insert = pgp.helpers.insert({ name }, ['name'], tableName)
        row = await db.one(insert)
      }
